...
{
public:
  FileSystemSearch();                     //constructor
  ~FileSystemSearch();               //destructor
...
void Block::push_back(Byte b)                   //push function
{
  bytes_.push_back(b);
}
//----< non-const indexer >------------------------------------------------
...
bool Block::operator!=(const Block& block) const            // operator
{
  return bytes_ != block.bytes_;
}
//----< return number of bytes in block >----------------------------------

size_t Block::size() const                //find the size
{
  return bytes_.size();
}
